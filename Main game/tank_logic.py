...
class TankLogic:
    ...
    @staticmethod
    def check_collision_with_items(tank, item,item_name,map_data,title_size, disappeared_items):
        res=None
        for tmp in item:
            ...
            if check is not None:
                print(f"Tank picked up {tmp[0]}")

                map_data[tmp[2]//title_size][tmp[1]//title_size]= '0'
                res=tmp[0]
                item.remove(tmp)
                disappeared_items.append((tmp, time.time()))
                break
        return res
    @staticmethod
    def respawn_items(item, map_data, tile_size, disappeared_items):
        # Check if any items should be respawned after 7 seconds
        current_time = time.time()
        for tmp in disappeared_items[:]:  # Iterate over a copy of the list
            if current_time - tmp[1] >= 7:  # 7 seconds have passed
                # Re-add the item at its original position
                item_type, x, y = tmp[0]  # Assuming tmp[0] is a tuple like (item_type, x, y)
                item.append((item_type, x, y))
                map_data[y // tile_size][x // tile_size] = str(item_type)
                disappeared_items.remove(tmp)
                print(item_type)
    @staticmethod
