...
import math
item = []
disappeared_items = []  # Each entry will be (item_type, x, y, disappear_time)
    ...
    def run(self, window):
        ...
        while self.running:
            ...
            for tank in self.tanks:
                ...
                if not tank.check :
                    tank.dx,tank.dy=0,0
                item_have=TankLogic.check_collision_with_items(tank,item,self.item_name,self.map_data,TILE_SIZE, disappeared_items)
                if item_have == 3:
            ...
            #Xuat hien lai cac vat pham da mat
            TankLogic.respawn_items(item, self.map_data, TILE_SIZE, disappeared_items)
