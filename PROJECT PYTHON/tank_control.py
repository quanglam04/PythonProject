...
class TankControl:
    ...
    def handle_input(self):#doc event hay doc input cua pygame
        ...
        if keys[self.control_setting['shoot']]:
            ...
            if self.shoot==False and (current_time-self.last_shoot) >500  and len(self.bullets) <4  :
                bullet = Bullet(
                    self.tank.tank_rect.centerx + 24 * math.cos(math.radians(self.tank.tank_angle)),
                    self.tank.tank_rect.centery - 24 * math.sin(math.radians(self.tank.tank_angle)),
